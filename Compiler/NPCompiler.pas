...
uses SysUtils, Classes, StrUtils, Types, NPCompiler.Parser, NPCompiler.Classes, NPCompiler.DataTypes, iDStringParser, IOUtils,
     IL.Types, IL.Instructions, AVL, NPCompiler.Operators, Math, NPCompiler.Errors, NPCompiler.Utils, Generics.Collections, HWPlatforms,
     NPCompiler.Intf, NPCompiler.Options;  // system, dateutils
...
type
  ...
  TSContext = record
    ...
    function CurUnit: TNPUnit; inline;
  ...
  {expression context - контекст выражения}
  TEContext = record
  private
    RPNLastItem: TOperatorID;        // последний входной элемент, для операндов = opNone
    RPNPrevPriority: Integer;        // приоритет предыдущей операции
  public
    SContext: PSContext;             // statement контекст
    ResultExpression: TIDExpression; // результатурющее выражение контекста
    StartInPos: Integer;             // стартовый индекс в RPN массиве операций для данного контекста
    StartOutPos: Integer;            // стартовый индекс в RPN массиве выражений для данного контекста
  ...
  var
    FRPNInput: TRPNItems;          // входной список операций
    FRPNInputSize: Integer;        // пердвычисленный размер входного списка
    FRPNInputPos: Integer;         // указывает на следющий свободный элемент входного списка
    FRPNOut: TIDExpressions;       // выходной список
    FRPNOutSize: Integer;          // пердвычисленный размер выходного списка
    FRPNOutPos: Integer;           // указывает на следющий свободный элемент выходного списка
    //////////////////////////////////////////////////////////////////////////////////////////
    ...
    function RPNFinish(var EContext: TEContext): TIDExpression;
    function RPNPushOperator(var EContext: TEContext; OpID: TOperatorID): TRPNStatus; overload;
    function RPNReadExpression(Index: Integer): TIDExpression; overload;
    function RPNLastOperator: TOperatorID;
  public
    function RPNPopExpression: TIDExpression; overload;
  private
  ...
  public
    ...
    function Process_operator_Period(SContext: PSContext): TIDExpression;
    function Process_operator_Addr(SContext: PSContext): TIDExpression;
    ...
    function Process_operator_Is(const SContext: PSContext): TIDExpression;
    function Process_operator_As(const SContext: PSContext): TIDExpression;
    ...
    function ProcessBuiltin_IncDec(SContext: PSContext; MacroID: TBuiltInFunctionID): TIDExpression;
    function ProcessBuiltin_Length(SContext: PSContext): TIDExpression;
    function ProcessBuiltin_SetLength(SContext: PSContext): TIDExpression;
    function ProcessBuiltin_Copy(SContext: PSContext): TIDExpression;
    function ProcessBuiltin_Move(SContext: PSContext): TIDExpression;
    function ProcessBuiltin_MemSet(SContext: PSContext): TIDExpression;
    function ProcessBuiltin_SizeOf(SContext: PSContext): TIDExpression;
    function ProcessBuiltin_Assert(SContext: PSContext; const ParamsTest: string; SourceRow: Integer): TIDExpression;
    function ProcessBuiltin_TypeName: TIDExpression;
    function ProcessBuiltin_New(Scontext: PSContext): TIDExpression;
    function ProcessBuiltin_Free(Scontext: PSContext): TIDExpression;
    function ProcessBuiltin_GetRef(SContext: PSContext): TIDExpression;
    function ProcessBuiltin_TypeInfo(SContext: PSContext): TIDExpression;
    function ProcessBuiltin_LoHiBound(SContext: PSContext; HiBound: Boolean): TIDExpression;
    function ProcessBuiltin_Ord(SContext: PSContext): TIDExpression;
    function ProcessBuiltin_Include(SContext: PSContext): TIDExpression;
    function ProcessBuiltin_Exclude(SContext: PSContext): TIDExpression;
    function ProcessBuiltin_RefCount(SContext: PSContext): TIDExpression;
    function ProcessBuiltin_GetBit(SContext: PSContext): TIDExpression;
    function ProcessBuiltin_SetBit(SContext: PSContext): TIDExpression;
    //======================================================================================================================================
    ...
    procedure CheckUnusedExprResult(const EContext: TEContext);
  ...
  public
    ...
    function ParseConstExpression(Scope: TScope; out Expr: TIDExpression; SrartToken: TTokenID; EPosition: TExpessionPosition): TTokenID;
    ...
    procedure ParseIIFStatement(Scope: TScope; SContext: PSContext);
    { функция парсинга вектор(кортеж) вид [a, b, ... z]}
    procedure ParseVector(Scope: TScope; SContext: PSContext);
    ...
    function CreateAnonymousConstant(Scope: TScope; const ID: TIdentifier; IdentifierType: TIdentifierType): TIDExpression;
  ...
  protected
    function Process(const SContext: PSContext): TIDExpression; virtual;
  end;
...
uses SystemUnit, NPCompiler.Messages, TypInfo, Platform.IL, NPCompiler.ConstCalculator;
...
var
  CVar: TIDVariable;
  ClosureClassExpr: TIDExpression;
  Node: TIDClosure.TCapturedVars.PAVLNode;
  AssignCode: TILMove;
  DstExpr, SrcExpr: TIDExpression;
  CVRec: ^TIDClosure.TCapturedVarRec;
begin
  CVar := SContext.Proc.GetTMPVar(Closure);
  Result := TIDExpression.Create(CVar, SContext.Proc.FirstBodyLine);
  ClosureClassExpr := TIDExpression.Create(Closure);
  // генерируем создание замыкания  (пока сразу в самом начале)
  ILWriteFirst(SContext, TIL.IL_DNew(Result, ClosureClassExpr));

  Node := Closure.CapturedVars.First;
  while Assigned(Node) do
  begin
    CVRec := addr(Node.Data);
    if not CVRec.ByRef then
    begin
      SrcExpr := TIDExpression.Create(CVRec.CapturedVar);
      DstExpr := TIDMultiExpression.Create(Result, TIDExpression.Create(CVRec.Field), Result.TextPosition);
      AssignCode := TIL.IL_Move(DstExpr, SrcExpr);
      ILWrite(SContext, AssignCode);
    end;
    Node := Closure.CapturedVars.Next(Node);
  end;
end;

procedure TNPUnit.EmitDynCheckBound(SContext: PSContext; ArrDecl: TIDDeclaration; Idx: TIDExpression);
var
  AExpr, TmpExpr: TIDExpression;
begin
  if ArrDecl.DataTypeID = dtStaticArray then
  begin
    AExpr := TIDExpression.Create(ArrDecl.DataType, Idx.TextPosition);
    ILWrite(SContext, TIL.IL_CheckBound(AExpr, Idx))
  end else begin
    AExpr := TIDExpression.Create(ArrDecl, Idx.TextPosition);
    if ArrDecl is TIDField then
    begin
      TmpExpr := GetTMPRefExpr(SContext, ArrDecl.DataType);
      ILWrite(SContext, TIL.IL_GetPtr(TmpExpr, nil, AExpr));
      AExpr := TmpExpr;
    end;
    ILWrite(SContext, TIL.IL_CheckBound(AExpr, Idx));
  end;
...
begin
  if Assigned(Expr) and Expr.IsTMPVar then
  begin
    if (mtNeedIncRef in Expr.DataType.ManagedFlags) and Expr.AsVariable.IsTemporaryOwner then
      SContext.ILWrite(TIL.IL_DecRef(Expr));
  end;
end;
...
procedure TNPUnit.RPNCheckInputSize;
begin
  if FRPNInputPos >= FRPNInputSize then begin
    Inc(FRPNInputSize, 8);
    SetLength(FRPNInput, FRPNInputSize);
  end;
end;
...
function TSContext.CurUnit: TNPUnit;
begin
  Result := TNPUnit(Proc.DeclUnit)
end;
...
function TNPUnit.MatchArrayImplicit(SContext: PSContext; Source: TIDExpression; DstArray: TIDArray): TIDExpression;
  ...
  begin
    Result := TIDDynArray.CreateAsAnonymous(FImplScope);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        TmpArrayType := TIDArray.CreateAnonymousStatic1Dim(FImplScope, DstElementDT, ACnt, BoundType);
        AddType(BoundType);
        AddType(TmpArrayType);
        Result := GetTMPVarExpr(SContext, TmpArrayType);
      end else begin
        // для динамических массивов просто выделяем память
        Result := GetTMPVarExpr(SContext, DstArray);
      ...
      for i := 0 to ACnt - 1 do begin
        ...
        RPNPushExpression(EContext, SExpr);
...
var
  TmpVar: TIDVariable;
  Result: TIDExpression;
begin
  if Source.ClassType = TIDDrefExpression then
  begin
    TmpVar := SContext.Proc.GetTMPVar(Source.DataType);
    Result := TIDExpression.Create(TmpVar, Source.TextPosition);
...
procedure TNPUnit.CheckUnusedExprResult(const EContext: TEContext);
var
  Expr: TIDExpression;
  ExprCnt: Integer;
begin
  Assert(EContext.StartOutPos >= 0);
  ExprCnt := (FRPNOutPos - EContext.StartOutPos);
  if ExprCnt > 0 then
  begin
    Expr := RPNReadExpression(ExprCnt - 1);
...
procedure TNPUnit.CheckAndCallRecordInit(const Proc: TIDProcedure; const Variable: TIDVariable);
var
  IL: TIL;
  Struct: TIDRecord;
  CallExpr: TIDCallExpression;
  ...
  InitCallCode,
  CtorCallCode: TILInstruction;
begin
  IL := TIL(Proc.IL);
  Struct := Variable.DataType as TIDRecord;
  // если определен инициализатор
  if Assigned(Struct.InitProc) then
  begin
    CallExpr := TIDCallExpression.Create(Struct.InitProc);
    SelfExpr := TIDExpression.Create(Variable);
    InitCallCode := TIL.IL_ProcCall(CallExpr, nil, SelfExpr, []);
    IL.InsertFirst(InitCallCode);
  end else
    InitCallCode := nil;
  // если определен статический конструктор для структуры
  if Assigned(Struct.StaticConstructor) then
  begin
    CallExpr := TIDCallExpression.Create(Struct.StaticConstructor);
    SelfExpr := TIDExpression.Create(Variable);
    CtorCallCode := TIL.IL_ProcCall(CallExpr, nil, SelfExpr, []);
    if Assigned(InitCallCode) then
      IL.InsertAfter(InitCallCode, CtorCallCode)
    else
      IL.InsertFirst(CtorCallCode);
  end;
end;
...
procedure TNPUnit.CheckAndCallRecordCopy(const Proc: TIDProcedure; const Variable: TIDVariable; InsetBefore: TILInstruction);
var
  IL: TIL;
  PExpr, Expr: TIDExpression;
  DataType: TIDType;
  CallCode: TILInstruction;
begin
  IL := TIL(Proc.IL);
  DataType := Variable.DataType;
  // если определен копировщик
  if Assigned(DataType.CopyProc) then
  begin
    PExpr := TIDCallExpression.Create(DataType.CopyProc);
    Expr := TIDExpression.Create(Variable);
    CallCode := TIL.IL_ProcCall(PExpr, nil, Expr, []);
    IL.InsertBefore(InsetBefore, CallCode)
  end;
end;

procedure TNPUnit.CheckAndCallRecordFinal(const Proc: TIDProcedure; const Variable: TIDVariable; InsetBefore: TILInstruction);
var
  IL: TIL;
  PExpr, Expr: TIDExpression;
  Struct: TIDRecord;
...
begin
  IL := TIL(Proc.IL);
  Struct := Variable.DataType as TIDRecord;
  // если определен статический деструктор для структуры
  if Assigned(Struct.StaticDestructor) then
  begin
    PExpr := TIDCallExpression.Create(Struct.StaticDestructor);
    Expr := TIDExpression.Create(Variable);
    CallCode := TIL.IL_ProcCall(PExpr, nil, Expr, []);
    if Assigned(InsetBefore) then
      IL.InsertBefore(InsetBefore, CallCode)
    else
      IL.Write(CallCode);
  end;
  // если определен финализатор
  if Assigned(Struct.FinalProc) then
  begin
    PExpr := TIDCallExpression.Create(Struct.FinalProc);
    Expr := TIDExpression.Create(Variable);
    CallCode := TIL.IL_ProcCall(PExpr, nil, Expr, []);
    if Assigned(InsetBefore) then
      IL.InsertBefore(InsetBefore, CallCode)
...
procedure TNPUnit.CheckAndCallArrayInit(const Proc: TIDProcedure; const Variable: TIDVariable);
var
  PExpr, Expr: TIDExpression;
  Arr: TIDArray;
  CallCode: TILInstruction;
begin
  Arr := Variable.DataType as TIDArray;
  if not (VarConstRef in Variable.Flags) and
     not (VarInOut in Variable.Flags) then
  begin
    // для любых массивов с инициализатором
    if Assigned(Arr.InitProc) then
    begin
      PExpr := TIDCallExpression.Create(Arr.InitProc);
      Expr := TIDExpression.Create(Variable);
      CallCode := TIL.IL_ProcCall(PExpr, nil, nil, [Expr]);
      if Arr.DataTypeID = dtStaticArray then
        TIL(Proc.IL).InsertFirst(CallCode)
      else
        TIL(Proc.IL).Write(CallCode);
     end else
     if (Arr.DataTypeID = dtStaticArray) and
        (Variable.Scope.ScopeType <> stGlobal) then
     begin
      // для стат. массивов локальных переменных или членов класса
       Expr := TIDExpression.Create(Variable);
       CallCode := TIL.IL_MemSet(Expr, SYSUnit._ZeroExpression);
       TIL(Proc.IL).InsertFirst(CallCode);
     end;
  end;
end;

procedure TNPUnit.CheckAndCallArrayCopy(const Proc: TIDProcedure; const Variable: TIDVariable; InsetBefore: TILInstruction);
var
  PExpr, Expr: TIDExpression;
  Arr: TIDArray;
  CallCode: TILInstruction;
begin
  Arr := Variable.DataType as TIDArray;
  if not (VarConstRef in Variable.Flags) and
     not (VarInOut in Variable.Flags) and
     Assigned(Arr.CopyProc) then
  begin
    PExpr := TIDCallExpression.Create(Arr.CopyProc);
    Expr := TIDExpression.Create(Variable);
    CallCode := TIL.IL_ProcCall(PExpr, nil, nil, [Expr]);
    TIL(Proc.IL).InsertBefore(InsetBefore, CallCode)
  end;
end;

procedure TNPUnit.CheckAndCallArrayFinal(const Proc: TIDProcedure; const Variable: TIDVariable; InsetBefore: TILInstruction);
var
  PExpr, Expr: TIDExpression;
  Arr: TIDArray;
  CallCode: TILInstruction;
begin
  ...
  if not (VarConstRef in Variable.Flags) and
     not (VarInOut in Variable.Flags) and
     Assigned(Arr.FinalProc) then
  begin
    PExpr := TIDCallExpression.Create(Arr.FinalProc);
    Expr := TIDExpression.Create(Variable);
    CallCode := TIL.IL_ProcCall(PExpr, nil, nil, [Expr]);
    if Assigned(InsetBefore) then
      TIL(Proc.IL).InsertBefore(InsetBefore, CallCode)
...
var
  Field: TIDDeclaration;
  DataTypeID: TDataTypeID;
  Dst, DefaultValue: TIDExpression;
  RecordType: TIDRecord;
begin
  Field := VarSpace.First;
  while Assigned(Field) do begin
    DataTypeID := Field.DataTypeID;
    DefaultValue := TIDVariable(Field).DefaultValue;
    {если есть значение по умолчанию, и это не параметр процедуры, то генерируем инициализацию}
    if Assigned(DefaultValue) and not (VarParameter in TIDVariable(Field).Flags)then
    begin
      if Assigned(ParentDecl) then
      begin
        Dst := TIDMultiExpression.CreateAnonymous(ParentDecl.Items);
        TIDMultiExpression(Dst).AddItem(TIDExpression.Create(Field));
      end else
        Dst := TIDExpression.Create(Field);
      ILWrite(SContext, TIL.IL_Move(Dst, DefaultValue));
    end else
    if DataTypeID = dtRecord then
    begin
      RecordType := TIDRecord(Field.DataType);
      if RecordType.HasInitFiels then
      begin
        if Assigned(ParentDecl) then
        begin
          Dst := TIDMultiExpression.CreateAnonymous(ParentDecl.Items);
          ...
          Dst := TIDMultiExpression.CreateAnonymous(Field);
        CheckInitVariables(SContext, TIDMultiExpression(Dst), RecordType.Members.VarSpace);
      end;
    end;
    Field := Field.NextItem;
  end;
...
begin
  ...
  FPackage := Package;
  FParser := TNPParser.Create(Source);
...
function TNPUnit.CreateAnonymousConstant(Scope: TScope; const ID: TIdentifier; IdentifierType: TIdentifierType): TIDExpression;
var
  IntValue: Int64;
  FltValue: Double;
  DataType: TIDType;
  ...
  case IdentifierType of
    ...
    itInteger: begin
      if Value[1] = '#' then begin
        ...
        begin
          if (RPNLastOperator = TOperatorID.opNegative) then
          begin
...
var
  ...
  IDItem: TIDItem;
  CurrNameItem: PIDItem;
  NextItem: PIDItem;
  NamesCnt: Integer;
begin
  CurrNameItem := addr(IDItem);
  while True do begin
    ...
    NamesCnt := 1;
    while True do begin
      parser_ReadCurrIdentifier(CurrNameItem.ID); // read name
      Result := parser_NextToken(Scope);
      if Result = token_coma then begin
        Result := parser_NextToken(Scope);
        parser_MatchParamNameIdentifier(Result);
        New(NextItem);
        CurrNameItem.NextItem := NextItem;
        CurrNameItem := NextItem;
        Inc(NamesCnt);
        Continue;
      end;
      ...
      begin
        ...
        // спецификаторы NOT NULL/NULLABLE
        case Result of
        ...
        if Result = token_equal then
          Result := ParseConstExpression(Scope, DefaultExpr, parser_NextToken(Scope), ExprNested)
        else
          DefaultExpr := nil;
      end else begin
      ...
      CurrNameItem := addr(IDItem);
      for i := 0 to NamesCnt - 1 do begin
        Param := TIDVariable.Create(Scope, CurrNameItem.ID, DataType, VarFlags);
        ...
        if DataType.DataTypeID = dtOpenArray then
        begin
          Param := TIDVariable.CreateAsSystem(Scope, CurrNameItem.ID.Name + '$Length');
          Param.DataType := SYSUnit._UInt32;
          Param.Flags := [VarParameter, VarHiddenParam, VarConst];
          Scope.AddVariable(Param);
        end;
        CurrNameItem := CurrNameItem.NextItem;
      end;

      CurrNameItem := IDItem.NextItem;
      for i := 0 to NamesCnt - 2 do
      begin
        NextItem := CurrNameItem;
        CurrNameItem := CurrNameItem.NextItem;
        Dispose(NextItem);
      end;
      Break;
...
function TNPUnit.ParseBindFunctionExpression(Scope: TScope; var EContext: TEContext): TTokenID;
var
  Idx: Integer;
  ProcID, ID: TIdentifier;
  Proc: TIDProcedure;
  Struct: TIDStructure;
  Expr: TIDExpression;
  ProcScope: TProcScope;
  GParams: TIDTypeList;
  ResultProc: TIDProcedure;
  Param: TIDVariable;
  ProcCode: TIL;
  CallCode: TILProcCall;
  ProcExpr: TIDCallExpression;
  CallArgs: TIDExpressions;
  CallArg: TIDExpression;
begin
  Struct := nil;
  Result := ParseProcName(Scope, ProcID, Struct, ProcScope, GParams);
  Proc := TIDProcedure(FindID(Scope, ProcID));

  if Proc.ItemType <> itProcedure then
    ERROR_PROC_REQUIRED(parser_PrevPosition);

  // создаем анонимную процедуру-переходник
  ResultProc := TIDProcedure.CreateAsSystem(FImplScope, '$bind_' + Proc.Name + IntToStr(Proc.Index));
  ResultProc.FirstBodyLine := parser_Line;
  ResultProc.LastBodyLine := parser_Line;
  FImplScope.AddAnonymousProcedure(ResultProc);

  // устанавливаем выходной параметр Result если это функция
  if Assigned(Proc.ResultType) then
    ResultProc.SetResult(Proc.ResultType);

  // чтение существующей сигнатуры
  ...
  begin
    ...
    while true do begin
      ...
      if FParser.IdentifireType = itIdentifier then
      begin
        Param := TIDVariable.Create(ResultProc.ParamsScope, ID);
        if not Assigned(Proc.NextOverload) then
        begin
          Param.DataType := Proc.ExplicitParams[Idx].DataType;
          Param.Flags := Proc.ExplicitParams[Idx].Flags;
        end;
        ResultProc.AddParam(Param);
        CallArg := TIDExpression.Create(Param, ID.TextPosition);
      end else
        CallArg := CreateAnonymousConstant(Scope, ID, FParser.IdentifireType);
  ...
  Result := parser_NextToken(Scope);
  // чтение новой сигнатуры
  if Result = token_to then
  begin

  end;
  parser_MatchSemicolon(Result);

  // генерация кода процедуры-переходника
  ProcExpr := TIDCallExpression.Create(Proc, ProcID.TextPosition);
  ProcCode := TIL.Create(ResultProc);
  CallCode := TIL.IL_ProcCall(ProcExpr, ResultProc.ResultParamExpression, nil, CallArgs);
  ProcCode.Write(CallCode);
  ResultProc.IL := ProcCode;
  ProcCode.Complete(FRCPathCount);
  ResultProc.Flags := ResultProc.Flags + [pfCompleted];
  ResultProc.CreateProcedureTypeIfNeed(Scope);
  AddType(ResultProc.DataType);


  Expr := TIDExpression.Create(ResultProc, ResultProc.TextPosition);
  RPNPushExpression(EContext, Expr);
end;
...
begin
  ...
  while True do begin
    ...
    Expr := InnerEContext.ResultExpression;
...
function TNPUnit.ParseCondMacro(Scope: TScope): TTokenID;
var
  ID: TIdentifier;
  Decl: TIDUserDefinedMacro;
  SContext: TSContext;
  Params: TScope;
  BPos: TParserPosition;
begin
  parser_ReadNextIdentifier(Scope, ID);

  Decl := TIDUserDefinedMacro.Create(Scope, ID);
  InsertToScope(Scope, Decl);

  Result := parser_NextToken(Scope);
  if Result = token_openround then
  begin
    Params := TProcScope.CreateInDecl(Scope, nil, nil);
    Params.VarSpace := addr(Decl.VarSpace);
    ParseParameters(Params, True);
    Decl.ParamsScope := Params;
    Decl.ExplicitParams := ScopeToVarList(Params, 0);
  ...
  FParser.SaveState(BPos);
  Decl.BodyPosition := BPos;

  parser_NextToken(Params);

  ParseStatements(Params, addr(SContext), False);

  parser_MatchSemicolon(Result);
  Result := parser_NextToken(Params);
end;

function TNPUnit.ParseCondEmit(Scope: TScope; SContext: PSContext): TTokenID;
var
  Expr: TIDExpression;
  PrevPos: TParserPosition;
  PrevSrc, NewSrc: string;
begin
  // парсим строковое выражение
  Result := ParseEmitExpression(Scope, Expr);

  CheckEmptyExpression(Expr);
  if Expr.DataTypeID = dtGeneric then
    Exit;

  // сохраняем положение в исходнике и сам исходник
  FParser.SaveState(PrevPos);
  PrevSrc := FParser.Source;

  if not (Expr.Declaration is TIDMacroArgument) then
  begin
    CheckStringExpression(Expr);
    NewSrc := Expr.AsConst.AsString;
  end else begin
    NewSrc := Expr.AsMacroArgument.Text;
  end;

  // загружаем новый исходник из выражения
  FParser.Source := NewSrc;
  FParser.First;
  FParser.NextToken;

  // компилируем новый исходник
  try
    ParseStatements(Scope, SContext, False);
  except
    on E: ECompilerAbort do begin
      E.CompilerMessage.Row := Expr.TextPosition.Row;
      E.CompilerMessage.Col := Expr.TextPosition.Col;
      raise;
    end;
  end;

  // восстанавливаем оригинальный исходник
  FParser.Source := PrevSrc;
  FParser.LoadState(PrevPos);
  Result := parser_CurTokenID;
end;
...
begin
  ...
  Result := ParseConstExpression(Scope, Expr, FParser.NextToken, ExprRValue);
...
begin
  ...
  while True do begin
    ...
    begin
      DataType := EContext.ResultExpression.DataType;
      for i := 0 to c do
...
begin
  ...
  case Token of
    ...
    token_cond_ifdef: begin
      Inc(FIFDEFCount);
      if not ParseCondIfDef(Scope) then
    ...
    token_cond_if: begin
      ...
      case ExprResult of
        condIFFalse: SkipToElseOrEnd(False);
        condIfTrue:;
        condIFUnknown: SkipToElseOrEnd(True);
      end;
    end;
    //////////////////////////////////////////
    // #else
    token_cond_else: begin
      if FIFDEFCount = 0 then
        ERROR_INVALID_COND_DIRECTIVE;
      Result := SkipToElseOrEnd(False);
    end;
    //////////////////////////////////////////
    // #end
    ...
    //////////////////////////////////////////
    // #include
    token_cond_include: begin
      ERROR_FEATURE_NOT_SUPPORTED;
      Result := token_unknown;
    end;
    token_cond_error: Result := ParseCondError(Scope);
    token_cond_warning: Result := ParseCondWarn(Scope);
    token_cond_hint: Result := ParseCondHint(Scope);
    token_cond_skip: Result := ParseCondSkip(Scope);
    //////////////////////////////////////////
    // delphi {$DEFINE ...}
    token_delphi_cond_define: begin
      ParseCondDefine(Scope, True);
      Result := FParser.NextToken;
    ...
    //////////////////////////////////////////
    // delphi {$UNDEFINE ...}
    token_delphi_cond_undefine: begin
      ParseCondDefine(Scope, False);
      Result := FParser.NextToken;
      parser_MatchToken(Result, token_closefigure);
      Result := parser_NextToken(Scope);
    ...
    //////////////////////////////////////////
    // delphi {$IFDEF ...}
    token_delphi_cond_ifdef: begin
      Inc(FIFDEFCount);
      if not ParseDelphiCondIfDef(Scope) then
        Result := SkipToElseOrEnd(False)
      else
        Result := parser_NextToken(Scope);
    end;
    //////////////////////////////////////////
    // delphi {$IFNDEF ...}
    token_delphi_cond_ifndef: begin
      Inc(FIFDEFCount);
      if ParseDelphiCondIfDef(Scope) then
        Result := SkipToElseOrEnd(False)
      else
        Result := parser_NextToken(Scope);
    end;
    ...
    token_cond_opt_set: Result := ParseCondOptSet(Scope);
    token_cond_opt_push: Result := ParseCondOptPush(Scope);
    token_cond_opt_pop: Result := ParseCondOptPop(Scope);
    token_cond_breakpoint: begin
      BP.Line := FParser.Position.Row;
      FBreakPoints.Add(BP);
      Result := parser_NextToken(Scope);
    end;
  else
...
procedure TNPUnit.ParseVector(Scope: TScope; SContext: PSContext);
...
begin
  ...
  while True do begin
    InitEContext(EContext, SContext, ExprNested);
    Token := ParseExpression(Scope, EContext, FParser.NextToken);
    Expr := EContext.ResultExpression;
    if Assigned(Expr) then begin
      if Expr.Declaration.ItemType <> itConst then
        IsStatic := False;
      Inc(c);
      SItems[i] := RPNPopExpression;
    end else
    if i > 0 then
      CheckEmptyExpression(Expr);
...
function TNPUnit.ParseConstExpression(Scope: TScope; out Expr: TIDExpression; SrartToken: TTokenID; EPosition: TExpessionPosition): TTokenID;
var
  EContext: TEContext;
  SContext: TSContext;
begin
  SContext.Initialize;
  SContext.WriteIL := False;
  InitEContext(EContext, addr(SContext), EPosition);
  Result := ParseExpression(Scope, EContext, SrartToken);
...
begin
  ...
  while True do begin
    case Result of
      token_eof: Exit;// ERROR_END_OF_FILE;
      ...
      token_iif: begin
        ParseIIFStatement(Scope, SContext);
        Status := rpOperand;
      end;
      token_openblock: begin
        ParseVector(Scope, SContext);
        Status := rpOperand;
      end;
      ...
      token_identifier: begin
        ...
        begin
          ...
          {анонимная константа}
          parser_ReadCurrIdentifier(ID);
          Expr := CreateAnonymousConstant(Scope, ID, parser_IdentifireType);
          Result := parser_NextToken(Scope);
          Status := rpOperand;
        end;
        RPNPushExpression(EContext, Expr);
  ...
  if (EContext.EPosition <> ExprNested) and (Status <> rpOperand) and NeedRValue(EContext.RPNLastItem) then
    ERROR_EXPRESSION_EXPECTED;

  EContext.ResultExpression := RPNFinish(EContext);
end;
...
function TNPUnit.ParseAsmSpecifier(out Platform: TIDPlatform): TTokenID;
  ...
  begin
    Result := HWPlatforms.FindPlatform(ID.Name);
    if not Assigned(Result) then
...
var
  ...
  PLInstructions: TILPlatform;
  Instr: TPlatformInstruction;
  Context: THWPContext;
begin
  //////////////////////////////////////////////////////////////////////////////
  Token := parser_CurTokenID;
  PLInstructions := TILPlatform.Create(Platform, parser_Line);
  Context.Init;
  Context.Scope := Scope;
  Context.Parser := FParser;
  ...
  try
    while True do begin
      case Token of
        token_identifier: begin
          parser_ReadCurrIdentifier(ID);
          Instr := Platform.FindInstruction(ID.Name);
          if not Assigned(Instr) then
          begin
            if parser_NextToken(Scope) = token_colon then
            begin
              Context.AddLabel(ID, Context.Stream.Position);
            end else
              AbortWork(sUnknownInstructionFmt, [ID.Name], parser_Position);
          end else begin
            PLInstructions.Add(Instr);
            if Instr.InstructionType = itInstruction then
            begin
              Instr.ParseArgumnets(Context);
              Instr.CheckArguments(Context);
              Instr.Encode(Context.Stream);
            end;
          end;
          Token := parser_NextToken(Scope);
        end;
    ...
    if Platform.Name = 'IL' then
    begin
      Instr := PLInstructions.FirstInstruction;
      while Assigned(Instr) do
      begin
        ILInstr := TILPInstruction(Instr).ILInstruction;
        Assert(Assigned(ILInstr));
        ILWrite(SContext, ILInstr);
        Instr := Instr.Next;
      end;
    end else
    if PLInstructions.Count > 0 then
      ILWrite(SContext, PLInstructions);
  finally
    Context.Destroy;
  end;
end;
...
begin
  ...
  Result := ParseExpression(Scope, EContext, parser_NextToken(Scope));
  Expression := EContext.ResultExpression;
...
procedure TNPUnit.ParseIIFStatement(Scope: TScope; SContext: PSContext);
  procedure ProcessSimpleMove(const EContext: TEContext; Dst, Src: TIDExpression);
  begin
    if (FRPNOutPos = 1) and Assigned(Src.Instruction) and
       (TILInstruction(Src.Instruction) = SContext.IL.Last) and
...
begin
  ...
  InitEContext(EContext, SContext, ExprRValue);
  Result := ParseExpression(Scope, EContext, parser_NextToken(Scope));
  AExpr := EContext.ResultExpression;
...
begin
  ...
  // начальное значение
  Result := ParseExpression(Scope, EContext, parser_NextToken(Scope));
  StartExpr := EContext.ResultExpression;
  ...
  InitEContext(EContext, SContext, ExprRValue);
  Result := ParseExpression(Scope, EContext, parser_NextToken(Scope));
  StopExpr := EContext.ResultExpression;
  ...
  if Result = token_step then begin
    InitEContext(EContext, SContext, ExprRValue);
    Result := ParseExpression(Scope, EContext, parser_NextToken(Scope));
    DeltaExpr := EContext.ResultExpression;
...
begin
  ...
  while true do begin
    ...
    Args[ArgsCount - 1] := RPNPopExpression;
...
begin
  ...
  while True do begin
    ...
    if EContext.ResultExpression.ExpressionType = etDeclaration then
    begin
      Expression := EContext.ResultExpression;
    end else begin
    // Если выражение сложное, создаем временную переменную и присваеваем ей вычисленное выше выражение
      ...
      TmpVar := SContext.Proc.GetTMPVar(EContext.ResultExpression.DataType, True);
...
begin
  ...
  case MacroID of
    bf_sysrtfunction: Expr := TIDSysRuntimeFunction(FuncDecl).Process(SContext);
    ...
    bf_inc, bf_dec: Expr := ProcessBuiltin_IncDec(SContext, MacroID);
    bf_length: Expr := ProcessBuiltin_Length(SContext);
    bf_setlength: Expr := ProcessBuiltin_SetLength(SContext);
    bf_copy: Expr := ProcessBuiltin_Copy(SContext);
    bf_move: Expr := ProcessBuiltin_Move(SContext);
    bf_memset: Expr := ProcessBuiltin_MemSet(SContext);
    bf_sizeof: Expr := ProcessBuiltin_SizeOf(SContext);
    bf_assert: Expr := ProcessBuiltin_Assert(SContext, ParamsText, ParamsBeginRow);
    bf_typename: Expr := ProcessBuiltin_TypeName;
    ...
    bf_new: Expr := ProcessBuiltin_New(SContext);
    bf_free: Expr := ProcessBuiltin_Free(SContext);
    bf_getref: Expr := ProcessBuiltin_GetRef(SContext);
    bf_typeinfo: Expr := ProcessBuiltin_TypeInfo(SContext);
    bf_LoBound: Expr := ProcessBuiltin_LoHiBound(SContext, False);
    bf_HiBound: Expr := ProcessBuiltin_LoHiBound(SContext, True);
    bf_Ord: Expr := ProcessBuiltin_Ord(SContext);
    bf_include: Expr := ProcessBuiltin_Include(SContext);
    bf_exclude: Expr := ProcessBuiltin_Exclude(SContext);
    bf_refcount: Expr := ProcessBuiltin_RefCount(SContext);
    bf_getbit: Expr := ProcessBuiltin_GetBit(SContext);
    bf_setbit: Expr := ProcessBuiltin_SetBit(SContext);
  else
    ...
    begin
      ...
      while true do begin
        ...
        begin
          ...
          RPNPopExpression; // убираем из стека
        end;
...
begin
  ...
  while True do begin
    ...
    if Token = token_equal then begin
      Token := ParseConstExpression(Scope, Expr, parser_NextToken(Scope), ExprNested);
      CheckEmptyExpression(Expr);
      CheckConstExpression(Expr);
...
function TNPUnit.ParseProcedure(Scope: TScope; ProcType: TProcType; Struct: TIDStructure; Platform: TIDPlatform): TTokenID;
type
...
begin
  InitEContext(EContext, SContext, ExprRValue);
  Result := ParseExpression(Scope, EContext, parser_NextToken(Scope));
  EExcept := EContext.ResultExpression;
...
var
  ...
  GUID: TGUID;
begin
  Result := ParseConstExpression(Scope, UID, parser_NextToken(Scope), ExprNested);
...
begin
  ...
  case Result of
    ...
    token_type: begin
      parser_MatchToken(parser_NextToken(Scope), token_of);
      Result := ParseTypeOf(Scope, Decl);
      // если не анонимное обьявление - создаем алиас
      if not IsAnonimous then begin
        Decl := TIDAliasType.CreateAlias(Scope, ID, Decl);
        InsertToScope(Scope, Decl);
      end;
      Exit;
    end;
    ...
    token_identifier: begin
      ...
      Result := ParseExpression(Scope, EContext, Result);
      {alias type}
      if EContext.ResultExpression.ItemType = itType then begin
        Decl := TIDAliasType.CreateAlias(Scope, ID, EContext.ResultExpression.AsType);
      end else
      {range type}
      begin
        Decl := TIDRangeType.Create(Scope, ID);
        ParseRangeType(Scope, EContext.ResultExpression, TIDRangeType(Decl));
      end;
      if not IsAnonimous then
        InsertToScope(Scope, Decl);
    end;
...
function TNPUnit.ParseNameSpace(Scope: TScope): TTokenID;
var
  ID: TIdentifier;
  NameSpaceDecl: TIDNameSpace;
  NameSpaceScope: TScope;
begin
  parser_ReadNextIdentifier(Scope, ID);
  NameSpaceDecl := TIDNameSpace.Create(Scope, ID);
  InsertToScope(Scope, NameSpaceDecl);
  Result := parser_NextToken(Scope);
  parser_MatchToken(Result, token_begin);
  Result := parser_NextToken(Scope);
  NameSpaceScope := NameSpaceDecl.Members;
  while True do begin
    case Result of
      token_type: Result := ParseNamedTypeDecl(NameSpaceScope);
      token_uses: Result := ParseUsesSection(NameSpaceScope);
      token_function: Result := ParseProcedure(NameSpaceScope, ptFunc);
      token_procedure: Result := ParseProcedure(NameSpaceScope, ptProc);
      token_namespace: Result := ParseNameSpace(NameSpaceScope);
      token_const: Result := ParseConstSection(NameSpaceScope);
      token_weak: begin
        parser_NextToken(NameSpaceScope);
        Result := ParseVarSection(NameSpaceScope, vLocal, nil, True);
      end;
      token_var: begin
        parser_NextToken(NameSpaceScope);
        Result := ParseVarSection(NameSpaceScope, vLocal, nil, False);
      end;
      token_end: begin
        Break;
      end;
...
begin
  ...
  while true do begin
    ...
    begin
      Result := ParseConstExpression(Scope, Expr, Result, ExprNested);
...
begin
  while true do begin
    ...
    UUnit := FPackage.UsesUnit(ID.Name, Self) as TNPUnit;
...
function TNPUnit.Process_operator_Addr(SContext: PSContext): TIDExpression;
var
  Expr: TIDExpression;
  TmpDecl: TIDVariable;
  DataType: TIDType;
...
begin
  ...
  begin
    Result := GetTMPVarExpr(SContext, DataType, Expr.TextPosition);
    Result.AsVariable.Absolute := Expr.AsVariable;
  end else begin
    TmpDecl := GetTMPVar(SContext, DataType);
    ...
    ILWrite(SContext, LEAInstruction);
  end;
...
function TNPUnit.ProcessBuiltin_Copy(SContext: PSContext): TIDExpression;
var
...
begin
  ...
  Cnt := RPNPopExpression;
  // читаем второй аргумент
  From := RPNPopExpression;
  // читаем первый аргумент
  Arr := RPNPopExpression;
  ...
  TmpVar := GetTMPVar(SContext, Arr.DataType);
  ...
  begin
    ...
    begin
      ...
      Cnt := GetTMPVarExpr(SContext, SYSUnit._Int32, parser_PrevPosition);
  ...
  begin
    Len := GetTMPVarExpr(SContext, SYSUnit._Int32, parser_PrevPosition);
    ILWrite(SContext, TIL.IL_Length(Len, Arr))
  end else
  ...
  begin
    ILWrite(SContext, TIL.IL_Cmp(From, SYSUnit._ZeroExpression));
    ILWrite(SContext, TIL.IL_Move(cLess, From, SYSUnit._ZeroExpression));

    ILWrite(SContext, TIL.IL_Cmp(From, Len));
    ILWrite(SContext, TIL.IL_Move(cGreater, From, Len));
  end;
  // проверка Cnt на значение < 0
  if not Cnt.IsConstant then
  begin
    ILWrite(SContext, TIL.IL_Cmp(Cnt, SYSUnit._ZeroExpression));
    ILWrite(SContext, TIL.IL_Move(cLess, Cnt, SYSUnit._ZeroExpression));
  end;

  if (not From.IsConstant) or (From.AsIntConst.Value > 0) then
  begin
    NCnt := GetTMPVarExpr(SContext, SYSUnit._Int32, parser_PrevPosition);
    ILWrite(SContext, TIL.IL_Sub(NCnt, Len, From));
    ILWrite(SContext, TIL.IL_Cmp(NCnt, Cnt));
    ILWrite(SContext, TIL.IL_Move(cGreater, NCnt, Cnt)); {}
  end else
    NCnt := Cnt;
  ...
  ReleaseExpression(SContext, Arr);
  ReleaseExpression(SContext, From);
  ReleaseExpression(SContext, Cnt);
end;

function TNPUnit.ProcessBuiltin_MemSet(SContext: PSContext): TIDExpression;
var
  Value, BytePattern: TIDExpression;
begin
  // читаем второй аргумент
  BytePattern := RPNPopExpression;
  CheckEmptyExpression(BytePattern);

  // читаем первый аргумент
  Value := RPNPopExpression;
  CheckEmptyExpression(Value);

  ILWrite(SContext, TIL.IL_MemSet(Value, BytePattern));
  ReleaseExpression(BytePattern);
  ReleaseExpression(Value);
  Result := nil;
end;

function TNPUnit.ProcessBuiltin_Move(SContext: PSContext): TIDExpression;
var
...
begin
  ...
  DstIdx := RPNPopExpression;
  // читаем третий аргумент
  DstArr := RPNPopExpression;
  // читаем второй аргумент
  SrcIdx := RPNPopExpression;
  // читаем первый аргумент
  SrcArr := RPNPopExpression;
...
function TNPUnit.ProcessBuiltin_IncDec(SContext: PSContext; MacroID: TBuiltInFunctionID): TIDExpression;
var
...
begin
  ...
  begin
    ...
    begin
      if Increment.IsConstant then
        ...
        NewIncrement := GetTMPVarExpr(SContext, SYSUnit._Int32);
  ...
  ReleaseExpression(SContext, Value);
  ReleaseExpression(SContext, Increment);

  Result := nil;
end;

function TNPUnit.ProcessBuiltin_Length(SContext: PSContext): TIDExpression;
var
  Expr: TIDExpression;
  DataType: TIDType;
  Decl: TIDDeclaration;
  ...
  case DataType.DataTypeID of
    ...
    dtDynArray, dtString, dtAnsiString: begin
      if Expr.Declaration is TIDDynArrayConstant then
        ...
        Result := TIDExpression.Create(SContext.Proc.GetTMPVar(SYSUnit._UInt32), Expr.TextPosition);
        ILWrite(SContext, TIL.IL_Length(Result, Expr));
      end;
    end;
    else begin
...
function TNPUnit.ProcessBuiltin_SetLength(SContext: PSContext): TIDExpression;
var
...
begin
  ...
  LenExpr := RPNPopExpression;
  // читаем первый аргумент (массив/строка)
  ArrExpr := RPNPopExpression;
  ...
  ReleaseExpression(SContext, LenExpr);
  Result := nil;
end;

function TNPUnit.ProcessBuiltin_Assert(SContext: PSContext; const ParamsTest: string; SourceRow: Integer): TIDExpression;
var
...
begin
  ...
  ETextExpr := RPNPopExpression;
  // читаем первый аргумент (утверждение)
  AssertExpr := RPNPopExpression;
  ...
  ReleaseExpression(SContext, AssertExpr);
  ReleaseExpression(SContext, ETextExpr);

  Result := nil;
end;

function TNPUnit.ProcessBuiltin_SizeOf(SContext: PSContext): TIDExpression;
var
  Expr: TIDExpression;
  Decl: TIDDeclaration;
  DataType: TIDType;
begin
  ...
  if Decl.ItemType <> itType then
    ...
    DataType := TIDType(Decl).ActualDataType;
  if not DataType.IsReferenced and (DataType.DataSize > 0) then
...
function TNPUnit.ProcessBuiltin_New(Scontext: PSContext): TIDExpression;
var
...
function TNPUnit.ProcessBuiltin_Free(Scontext: PSContext): TIDExpression;
var
  Expr, PExpr: TIDExpression;
  Code: TILInstruction;
  DataType: TIDType;
...
function TNPUnit.ProcessBuiltin_GetBit(SContext: PSContext): TIDExpression;
var
  Value, BitIndex: TIDExpression;
begin
  // читаем аргумент 2
  BitIndex := RPNPopExpression;
  CheckIntExpression(BitIndex);
  // читаем аргумент 1
  Value := RPNPopExpression;
  // результат функции
  Result := GetTMPVarExpr(SContext, SYSUnit._Boolean);
  ...
  ILWrite(SContext, TIL.IL_GetBit(Result, Value, BitIndex));

  ReleaseExpression(Value);
  ReleaseExpression(BitIndex);
end;

function TNPUnit.ProcessBuiltin_SetBit(SContext: PSContext): TIDExpression;
var
  Value, BitIndex, BitValue: TIDExpression;
begin
  // читаем аргумент 3
  BitValue := RPNPopExpression;
  ...
  ILWrite(SContext, TIL.IL_SetBit(Value, BitIndex, BitValue));
...
function TNPUnit.ProcessBuiltin_GetRef(SContext: PSContext): TIDExpression;
var
  Src, Dst: TIDExpression;
begin
  // читаем аргумент StrongPtr
  Dst := RPNPopExpression;
  // читаем аргумент WeakPtr
  Src := RPNPopExpression;
  ILWrite(Scontext, TIL.IL_StrongRef(Dst, Src));
  ILWrite(Scontext, TIL.IL_Cmp(Dst, SYSUnit._NullPtrExpression));
  Result := GetTMPVarExpr(SContext, SYSUnit._Boolean);
  ILWrite(Scontext, TIL.IL_SetBool(cNotEqual, Result));
end;

function TNPUnit.ProcessBuiltin_Include(SContext: PSContext): TIDExpression;
var
  ASet, ASubSet, SValue: TIDExpression;
  SetDT: TIDSet;
begin
  // читаем аргумент SubSet
  ASubSet := RPNPopExpression;
  // читаем аргумент Set
  ASet := RPNPopExpression;
  CheckSetType(ASet);
  ...
  case ASubSet.DataTypeID of
    dtSet: begin
      ...
      ILWrite(Scontext, TIL.IL_Or(ASet, ASet, ASubSet));
    end;
    dtEnum: begin
      ...
      begin
        ...
        SValue := nil;
      ILWrite(Scontext, TIL.IL_Or(ASet, ASet, SValue));
    end;
    dtDynArray: begin
...
function TNPUnit.ProcessBuiltin_Exclude(SContext: PSContext): TIDExpression;
var
  ASet, ASubSet, SValue: TIDExpression;
  SetDT: TIDSet;
begin
  ...
  case ASubSet.DataTypeID of
    dtSet: begin
      if ASubSet.DataType.ActualDataType <> SetDT then
        ERROR_INCOMPATIBLE_TYPES(ASubSet, SetDT.BaseType);
      SValue := GetTMPVarExpr(SContext, SetDT);
      ILWrite(Scontext, TIL.IL_Not(SValue, ASubSet));
      ILWrite(Scontext, TIL.IL_And(ASet, ASet, SValue));
    end;
    dtEnum: begin
      if ASubSet.DataType.ActualDataType <> SetDT.BaseType.ActualDataType then
        ERROR_INCOMPATIBLE_TYPES(ASubSet, SetDT.BaseType);
...
function TNPUnit.ProcessBuiltin_TypeInfo(SContext: PSContext): TIDExpression;
var
  Expr: TIDExpression;
begin
  // читаем аргумент
  Expr := RPNPopExpression;
  CheckEmptyExpression(Expr);
  Result := GetTMPVarExpr(SContext, SYSUnit._TObject);
  ILWrite(SContext, TIL.IL_TypeInfo(Result, Expr));
end;

function TNPUnit.ProcessBuiltin_Ord(SContext: PSContext): TIDExpression;
var
  Expr: TIDExpression;
  CValue: Int64;
begin
...
function TNPUnit.ProcessBuiltin_RefCount(SContext: PSContext): TIDExpression;
var
  Expr: TIDExpression;
begin
  // читаем аргумент
  ...
  Result := GetTMPVarExpr(SContext, SYSUnit._Int32);
  Result.TextPosition := parser_Position;

  ILWrite(SContext, TIL.IL_RefCount(Result, Expr));
end;

function TNPUnit.ProcessBuiltin_LoHiBound(SContext: PSContext; HiBound: Boolean): TIDExpression;
var
  Expr: TIDExpression;
  Decl: TIDDeclaration;
  DataType: TIDType;
  ...
  begin
    case HiBound of
      ...
      True: begin
        ...
        Result.TextPosition := parser_Position;
        ILWrite(SContext, TIL.IL_Length(Result, Expr));
        ILWrite(SContext, TIL.IL_Sub(Result, Result, SYSUnit._OneExpression));
        Exit;
      end;
    end;
  end else
...
function TNPUnit.ProcessBuiltin_TypeName: TIDExpression;
var
  Expr: TIDExpression;
  Decl: TIDDeclaration;
begin
...
begin
  ...
  PExpr := TIDCallExpression(RPNPopExpression);
  ...
  for AIndex := ArgsCount - 1 downto 0 do
    UserArguments[AIndex] := RPNPopExpression;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Inc(PIndex);
        CallArguments[PIndex] := ProcessBuiltin_Length(SContext);
      end;
...
begin
  ...
  begin
    ...
    if Left.IsConstant then
      LeftExpr := CreateAnonymousConstant(nil, Identifier(IntToStr(1 shl Left.AsIntConst.Value)), itInteger)
    else
      LeftExpr := Left;
...
function TNPUnit.Process_operator_Is(const SContext: PSContext): TIDExpression;
var
  Src, Dst: TIDExpression;
begin
  Dst := RPNPopExpression();
  ...
  Result := GetTMPVarExpr(SContext, SYSUnit._Boolean, Dst.TextPosition);
  ILWrite(SContext, TIL.IL_QueryType(Result, Src, Dst));
end;

function TNPUnit.Process_operator_As(const SContext: PSContext): TIDExpression;
var
  Src, Dst: TIDExpression;
begin
  Dst := RPNPopExpression();
  ...
  CheckClassOrIntfType(Src.DataType, Src.TextPosition);
  CheckClassOrIntfType(Dst);
  Result := GetTMPVarExpr(SContext, Dst.AsType, Dst.TextPosition);
  ILWrite(SContext, TIL.IL_QueryType(Result, Src, Dst));
end;
...
var
  ...
  i: Integer;
begin
  Source := RPNPopExpression;
  for i := EContext.StartOutPos to FRPNOutPos - 1 do
  begin
    {вычитываем очередной приемник (читаем слево-направо)}
    Dest := RPNReadExpression(i);
    ...
    if (FRPNOutPos - EContext.StartOutPos) = 1 then
    begin
  ...
  FRPNOutPos := EContext.StartOutPos;
end;
...
begin
  if (FRPNOutPos - EContext.StartOutPos) = 2 then
  begin
    Source := RPNPopExpression;
    Dest := RPNPopExpression;
...
begin
  ...
  case OpID of
    ...
    opPositive: Result := RPNPopExpression;
    opDereference: Result := Process_operator_Deref(EContext);
    opCall: Result := Process_CALL(EContext);
    opPeriod: Result := Process_operator_Period(SContext);
    opAddr: Result := Process_operator_Addr(SContext);
    opIs: Result := Process_operator_Is(SContext);
    opAs: Result := Process_operator_As(SContext);
    opPostInc: begin
      // Читаем левый операнд
      Left := RPNPopExpression;
    ...
    opPostDec: begin
      ...
      if (Left.IsConstant and Right.IsConstant) and (Left.Declaration.ClassType <> TIDSizeofConstant) and (Right.Declaration.ClassType <> TIDSizeofConstant) then
      begin
...
function TNPUnit.Process_operator_Period(SContext: PSContext): TIDExpression;
var
...
begin
  ...
  EContext.StartInPos := FRPNInputPos;
  EContext.StartOutPos := FRPNOutPos;
  if Assigned(SContext) then
    EContext.LastInstruction := SContext.ILLast
  else
    EContext.LastInstruction := nil;
  EContext.LastBoolNode := nil;
  EContext.ResultExpression := nil;
  EContext.EPosition := EPosition;
  EContext.RPNLastItem := opNone;
  EContext.RPNPrevPriority := 0;
end;
...
procedure TNPUnit.RPNError(Status: TRPNError);
begin
  with FParser do
  case Status of
    reUnclosedOpenBracket: AbortWork(sUnclosedOpenBracket, PrevPosition);
    reDublicateOperation: AbortWork(sDublicateOperationFmt, [TokenLexem(TTokenID(CurrentTokenID))], PrevPosition);
  end;
end;

function TNPUnit.RPNFinish(var EContext: TEContext): TIDExpression;
var
  op: TOperatorID;
begin
  while FRPNInputPos > EContext.StartInPos do
  begin
    Dec(FRPNInputPos);
    op := FRPNInput[FRPNInputPos];
    if op <> opOpenRound then
    begin
      Result := Process_operators(EContext, op);
      FRPNOut[FRPNOutPos] := Result;
      if Assigned(Result) then
        Inc(FRPNOutPos);
      if FRPNInputPos > 0 then
        EContext.RPNLastItem := FRPNInput[FRPNInputPos - 1]
      else
        EContext.RPNLastItem := opNone;
      EContext.RPNPrevPriority := cOperatorPriorities[EContext.RPNLastItem];
    end else
      RPNError(reUnclosedOpenBracket);
  end;

  if FRPNOutPos > EContext.StartOutPos then
    Result := FRPNOut[FRPNOutPos - 1]
  else
    Result := nil;
end;

function TNPUnit.RPNPushOperator(var EContext: TEContext; OpID: TOperatorID): TRPNStatus;
var
  Priority: Integer;
  Op: TOperatorID;
begin
  if OpID = EContext.RPNLastItem then
    RPNError(reDublicateOperation);

  EContext.RPNLastItem := OpID;

  Priority := cOperatorPriorities[OpID];
  if (Priority <= EContext.RPNPrevPriority) then begin
    while (FRPNInputPos - EContext.StartInPos) > 0 do begin
      Op := FRPNInput[FRPNInputPos - 1];
      if (cOperatorPriorities[Op] >= Priority) and (Op <> opOpenRound) then
      begin
        Dec(FRPNInputPos);
        FRPNOut[FRPNOutPos] := Process_operators(EContext, Op);
        Inc(FRPNOutPos);
      end else
        Break;
    end;
  end;
  EContext.RPNPrevPriority := Priority;
  FRPNInput[FRPNInputPos] := OpID;
  Inc(FRPNInputPos);
  RPNCheckInputSize;
  Result := rpOperation;
end;
...
var
  Op: TOperatorID;
begin
  if FRPNInputPos > EContext.StartInPos then
  begin
    Dec(FRPNInputPos);
    Op := FRPNInput[FRPNInputPos];
    Result := Process_operators(EContext, op);
...
var
  op: TOperatorID;
begin
  EContext.RPNLastItem := opCloseRound;
  while FRPNInputPos > 0 do begin
    Dec(FRPNInputPos);
    op := FRPNInput[FRPNInputPos];
...
begin
  FRPNInput[FRPNInputPos] := opOpenRound;
  Inc(FRPNInputPos);
  RPNCheckInputSize;
  EContext.RPNLastItem := opOpenRound;
end;

procedure TNPUnit.RPNPushExpression(var EContext: TEContext; Operand: TIDExpression);
begin
  FRPNOut[FRPNOutPos] := Operand;
  Inc(FRPNOutPos);
  if FRPNOutPos >= FRPNOutSize then begin
    Inc(FRPNOutSize, 8);
    SetLength(FRPNOut, FRPNOutSize);
  end;
  EContext.RPNLastItem := opNone;
...
function TNPUnit.RPNPopExpression: TIDExpression;
begin
  Dec(FRPNOutPos);
  if FRPNOutPos >= 0 then begin
    Result := FRPNOut[FRPNOutPos];
    if Assigned(Result) then
      Exit;
  end;
  ERROR_EMPTY_EXPRESSION;
...
function TNPUnit.RPNReadExpression(Index: Integer): TIDExpression;
begin
  {$IFDEF DEBUG}
  if (Index < 0) or (Index >= FRPNOutPos) then
    AbortWork(sInvalidIndex, FParser.Position);
  {$ENDIF}
  Result := FRPNOut[Index];
//  if (Result.ItemType = itVar) and Result.IsAnonymous then // Освобождаем временный Expression
//    TIDVariable(Result.Declaration).UseInExpression := False;
end;

function TNPUnit.RPNLastOperator: TOperatorID;
begin
  if FRPNInputPos > 0 then
    Result := FRPNInput[FRPNInputPos - 1]
  else
...
procedure TEContext.Initialize;
begin
  FillChar(Self, SizeOf(TEContext), #0);
end;
...
function TIDSysRuntimeFunction.Process(const SContext: PSContext): TIDExpression;
begin
