...
====================================================================*)
  ...
  TILPlatform = class(TILInstruction)
  private
    FPlatform: TIDPlatform;
    FCount: Integer;
    FFirst: TPlatformInstruction;
    FLast: TPlatformInstruction;
    FLine: Integer;
  protected
    function GetLine: Integer; override;
  public
    constructor Create(Platform: TIDPlatform; Line: Integer);
    destructor Destroy; override;
    procedure Write(Proc: TIDProcedure; Stream: TStream); override;
    procedure Add(Instruction: TPlatformInstruction);
    function ILCode: TILCode; override;
    function Text: string; override;
    property Count: Integer read FCount;
    property FirstInstruction: TPlatformInstruction read FFirst;
    property LastInstruction: TPlatformInstruction read FLast;
  end;
...
uses SystemUnit, NPCompiler, NPCompiler.Operators, NPCompiler.ConstCalculator;
...
{ TILPlatformInstructions }

procedure TILPlatform.Add(Instruction: TPlatformInstruction);
begin
  ...
  begin
    ...
    FLast.Next := Instruction;
    FLast := Instruction;
  end;
  Inc(FCount);
end;

constructor TILPlatform.Create(Platform: TIDPlatform; Line: Integer);
begin
  CreateFromPool;
  FPlatform := Platform;
  FLine := Line;
...
destructor TILPlatform.Destroy;
var
  Instr, Tmp: TPlatformInstruction;
begin
  Instr := FFirst;
  while Assigned(Instr) do begin
    Tmp := Instr;
    Instr := Instr.Next;
    Tmp.Free;
  end;
...
function TILPlatform.GetLine: Integer;
begin
  Result := FLine;
end;

function TILPlatform.ILCode: TILCode;
begin
  Result := icPlatform
end;

function TILPlatform.Text: string;
var
  Instruction: TPlatformInstruction;
  Prefix, sCode: string;
  Stream: TMemoryStream;
begin
  Result := '';
  Prefix := FPlatform.Name + ': ';
  Instruction := FFirst;
  Stream := TMemoryStream.Create;
  try
    while Assigned(Instruction) do begin
      Instruction.Encode(Stream);
      Result := AddStringSegment(Result, Prefix + Instruction.Text, #10);
      Instruction := Instruction.Next;
    end;
    Result := AddStringSegment(Result, 'bytes: ', #10);
    SetLength(sCode, Stream.Size*2);
    //BinToHex(Stream.Memory, {$IFDEF FPC}PAnsiChar{$ELSE}PChar{$ENDIF}(sCode), Stream.Size);
    Result := Result + sCode;
  finally
    Stream.Free;
  end;
...
procedure TILPlatform.Write(Proc: TIDProcedure; Stream: TStream);
var
  Instruction: TPlatformInstruction;
begin
  inherited Write(Proc, Stream);
  Stream.WriteStretchUInt(0); // platform name
  Stream.WriteStretchUInt(FCount);
  Instruction := FFirst;
  while Assigned(Instruction) do begin
